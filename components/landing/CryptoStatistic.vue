              ...
              <tr v-for="data in datasets" :key="data.id" class="border-b border-gray-200">
                <td class="py-4 whitespace-nowrap">
                  <div class="flex items-center space-x-2">
                    <img style="width: 25px; height: 25px;" :src="require(`~/assets/img/crypto-icon/${data.logo}`)" />
                    <span class="overflow-hidden text-ellipsis w-40 inline-block" :title="data.name">{{
                      data.name
                    }}</span>
                  </div>
                </td>
